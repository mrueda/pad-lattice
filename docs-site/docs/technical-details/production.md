...
The server retains its loopback administrator listener and adds a LAN listener
only on the selected private IPv4 interface. The tokenized local page displays
a five-minute, one-use QR code and PIN. Remote clients receive no agent state
until paired. Pairing tokens exist only for the current daemon process and can
be revoked from the local browser.
...
Surface](./virtual-surface.md) and the [Security
Model](./security-model.md) for the complete boundary.
...
Feedback](../usage/audio-feedback.md) for the complete vocabulary and local player
requirements.
